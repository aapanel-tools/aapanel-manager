...
describe('actionErrorCode', () => {
  it('recognises the codes actions actually return', () => {
    for (const code of ['unauthenticated', 'forbidden', 'validation', 'confirm', 'invalid', 'failed', 'notFound', 'auditUnavailable', 'outdated', 'unreachable']) {
...
describe('the shared refusal vocabulary', () => {
  const CODES = ['unauthenticated', 'forbidden', 'validation', 'confirm', 'invalid', 'failed', 'notFound', 'auditUnavailable', 'outdated', 'unreachable'];
